...
const grip = (b, c = WOOD, len = 0.13, r = 0.022) =>
...
const ITEMS = {
  ...
  blade: (b, a) => {
    grip(b, a.hilt || '#6b4a2f');
    b.box(-0.035, -0.012, -0.018, 0.035, 0.012, 0.018, tone(rgb(a.hilt || '#6b4a2f'), -0.12));
    blade(b, a.c || STEEL, 0.22, 0.026, 0.012);
  },

  saw: (b, a) => {
    grip(b, '#6b4a2f');
    const c = a.c || BONE;
    b.box(-0.022, 0, -0.008, 0.022, 0.20, 0.008, c);
    for (let i = 0; i < 7; i++) {                      // teeth
      b.box(0.022, 0.02 + i * 0.025, -0.007, 0.038, 0.032 + i * 0.025, 0.007, tone(rgb(c), -0.10));
    }
  },

  spear: (b, a) => {
    // gripped a third of the way up the shaft, which is where you would hold it
    b.drum(0, -0.30, 0, 0.020, 0.018, 0.50, WOOD, 6);
    b.drum(0, -0.32, 0, 0.026, 0.026, 0.03, '#5a4735', 6);      // the butt
    b.drum(0, 0.16, 0, 0.026, 0.022, 0.05, '#5a4735', 6);       // the socket
    blade(b, a.c || STEEL, 0.17, 0.030, 0.20);
  },

  pick: (b, a) => {
    grip(b, WOOD, 0.18, 0.02);
    const c = a.c || BONE;
    b.box(-0.015, 0, -0.015, 0.015, 0.05, 0.015, tone(rgb(c), -0.08));
    b.cone(0, 0.05, 0.02, 0.03, 0.14, c, 5);
    b.cone(0, 0.05, -0.02, 0.03, 0.11, tone(rgb(c), -0.05), 5);
  },

  needle: (b, a) => {
    b.drum(0, -0.02, 0, 0.008, 0.004, 0.20, a.c || STEEL, 5);
    b.drum(0, -0.04, 0, 0.014, 0.014, 0.02, tone(rgb(a.c || STEEL), -0.15), 6);
  },

  gaff: (b, a) => {
    grip(b, WOOD, 0.22, 0.018);
    b.drum(0, -0.22, 0, 0.018, 0.016, 0.34, WOOD, 6);
    // the hook the whole thing is named for
    const c = a.c || STEEL;
    b.drum(0, 0.12, 0, 0.012, 0.012, 0.06, c, 5);
    b.box(-0.01, 0.17, -0.01, 0.07, 0.19, 0.01, c);
    b.cone(0.07, 0.19, 0, 0.016, 0.05, tone(rgb(c), 0.08), 5, Math.PI);
  },

  /* -- blunt ----------------------------------------------- */

  hammer: (b, a) => {
    grip(b, WOOD, 0.20, 0.02);
    const c = a.c || '#5a5a62';
    b.box(-0.055, 0.0, -0.032, 0.055, 0.075, 0.032, c);
    b.box(-0.07, 0.012, -0.026, -0.055, 0.062, 0.026, tone(rgb(c), 0.06));
    b.box(0.055, 0.012, -0.026, 0.075, 0.062, 0.026, tone(rgb(c), -0.06));
  },

  staff: (b, a) => {
    b.drum(0, -0.26, 0, 0.019, 0.017, 0.50, a.c || WOOD, 6);
    b.ball(0, 0.27, 0, 0.045, a.gem || '#6fd1a5', 3, 6);
    for (let i = 0; i < 3; i++) {
      const t = i * 2.1;
      b.drum(Math.cos(t) * 0.03, 0.20, Math.sin(t) * 0.03, 0.01, 0.008, 0.06,
  ...
  bow: (b, a) => {
    const c = a.c || BONE;
    for (const s of [-1, 1]) {
      b.quad([0.03, s * 0.02, -0.01], [0.055, s * 0.14, -0.01],
             [0.055, s * 0.14, 0.01], [0.03, s * 0.02, 0.01], c);
      b.quad([0.055, s * 0.14, -0.01], [0.02, s * 0.26, -0.01],
             [0.02, s * 0.26, 0.01], [0.055, s * 0.14, 0.01], tone(rgb(c), -0.06));
    }
    b.box(0.028, -0.03, -0.014, 0.05, 0.03, 0.014, tone(rgb(c), -0.12));
    b.box(0.014, -0.26, -0.004, 0.022, 0.26, 0.004, '#5a4a3a');    // the string
  },

  blowpipe: (b, a) => {
    b.drum(0, -0.20, 0, 0.024, 0.020, 0.44, a.c || '#6b8f5f', 7);
    b.drum(0, -0.21, 0, 0.032, 0.032, 0.03, tone(rgb(a.c || '#6b8f5f'), -0.14), 7);
  },

  syringe: (b, a) => {
    const c = a.c || '#c9a34a';
    b.drum(0, -0.10, 0, 0.030, 0.030, 0.20, tone(rgb(c), 0.05), 7);
    b.drum(0, 0.10, 0, 0.010, 0.004, 0.14, tone(rgb(c), 0.12), 5);
    b.box(-0.045, -0.115, -0.012, 0.045, -0.10, 0.012, tone(rgb(c), -0.10));
    b.drum(0, -0.15, 0, 0.020, 0.020, 0.04, tone(rgb(c), -0.05), 6);
  },

  dart: (b, a) => {
    b.cone(0, 0.02, 0, 0.016, 0.11, a.c || STEEL, 5);
    b.drum(0, -0.08, 0, 0.010, 0.014, 0.10, tone(rgb(a.c || STEEL), -0.12), 5);
    for (let i = 0; i < 3; i++) {
      const t = i * 2.1;
      b.box(Math.cos(t) * 0.012, -0.09, Math.sin(t) * 0.012,
            Math.cos(t) * 0.035, -0.05, Math.sin(t) * 0.035, '#c0303f');
    }
  },

  net: (b) => {
    b.drum(0, -0.16, 0, 0.015, 0.013, 0.24, WOOD, 5);
    b.drum(0, 0.08, 0, 0.10, 0.11, 0.02, '#8a7c62', 8);
    b.drum(0, 0.02, 0, 0.03, 0.10, 0.07, '#6b6250', 8);
  },
